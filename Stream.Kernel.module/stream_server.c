...
#define MODULE_NAME "Distributed Multipath TCP Stream Server"
#define MAX_CONNS 16

MODULE_LICENSE("GPL");
MODULE_AUTHOR("CHRV");

static char * peer_server_addr;
module_param(peer_server_addr, charp,0666);

static char * file_name;
module_param(file_name, charp, 0666);
...
int connection_handler(void *data)
{
	...
	in_addr_t peer_addr = inet_addr(peer_server_addr)
	...
	while (1)
	{
		...
		ret = tcp_server_send(accept_socket, id, address, out_buff, len);
